...
public class MiningMinigame {
	...
	public MiningMinigame(Player player) {
		this.player = player;
		this.task = null;
		this.time = 0;
		this.tick = 8;
		...
		task = Bukkit.getScheduler().runTaskTimer(Core.getCore(), () -> {
			...
			if (this.time > this.maxTime) {
				...
				if (type != null && correct > 0) {
					...
					Item drop = this.player.getWorld().dropItem(this.player.getEyeLocation(), item);
				...
				player.sendTitle(msg, ChatColor.RED + "종료!", 0, 20, 0);
				return;
			}
			if (this.time % tick == 0) {
				this.now++;
