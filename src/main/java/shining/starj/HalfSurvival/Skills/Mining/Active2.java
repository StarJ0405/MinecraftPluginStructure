...
public class Active2 extends UsableSkill {
	...
	public boolean use(Player player) {
		if (super.use(player)) {
			...
			if (b == null || b.getType().equals(Material.AIR)) {
				...
				HashMapStore.setTransformMiddle2(player, true);
	...
	public void act(Block b, Player player) {
		...
		boolean transform_right2 = Skill.Mining.transform_right2.hasLearn(player);
		boolean upgrade_right2 = Skill.Mining.upgrade_right2.hasLearn(player);
								...
								if (transform_right2) {
									if (canBreak == 1) {
										...
										Bukkit.getScheduler().runTaskLater(Core.getCore(), () -> {
											now.getBlock().setType(Skill.Mining.active4.getRandomBlock(), true);
											if (upgrade_right2) {
												ConfigStore.<String>setBlockConfig(now.getBlock(), "type",
														Skill.Mining.active4.getKey());
												Integer minI = ConfigStore.<Integer>getBlockConfig(now.getBlock(),
														"min");
												int min = minI != null ? minI : 1;
												ConfigStore.<Integer>setBlockConfig(now.getBlock(), "min", min);
												Integer maxI = ConfigStore.<Integer>getBlockConfig(now.getBlock(),
														"max");
												int max = maxI != null ? maxI : 1;
												max += (int) Skill.Mining.upgrade_right2.getEffect();
												ConfigStore.<Integer>setBlockConfig(now.getBlock(), "max",
														max > 5 ? 5 : max);
											}
										}, 1);
										player.playSound(player, Sound.ITEM_INK_SAC_USE, 1f, 1f);
										player.getWorld().spawnParticle(Particle.SQUID_INK,
												now.clone().add(0.5, 0.5, 0.5), 10, 0.5f, 0.5f, 0.5f, 0.1f);
										if (upgrade_left4)
											list.add(type);
									}
								} else {
									if (canBreak == 2) {
										...
										if (upgrade_right2) {
											ConfigStore.<String>setBlockConfig(now.getBlock(), "type",
													Skill.Mining.active4.getKey());
											Integer minI = ConfigStore.<Integer>getBlockConfig(now.getBlock(), "min");
											int min = minI != null ? minI : 1;
											ConfigStore.<Integer>setBlockConfig(now.getBlock(), "min", min);
											Integer maxI = ConfigStore.<Integer>getBlockConfig(now.getBlock(), "max");
											int max = maxI != null ? maxI : 1;
											max += (int) Skill.Mining.upgrade_right2.getEffect();
											ConfigStore.<Integer>setBlockConfig(now.getBlock(), "max",
													max > 5 ? 5 : max);
						...
						if (canBreak > 0) {
							if (transform_right2) {
								if (canBreak == 1) {
									...
									player.playSound(player, Sound.ITEM_INK_SAC_USE, 1f, 1f);
									player.getWorld().spawnParticle(Particle.SQUID_INK, now.clone().add(0.5, 0.5, 0.5),
								...
								if (canBreak == 2) {
									...
									if (upgrade_right2) {
										...
										int max = maxI != null ? maxI : 1;
										max += (int) Skill.Mining.upgrade_right2.getEffect();
										ConfigStore.<Integer>setBlockConfig(now.getBlock(), "max", max > 5 ? 5 : max);
									}
								}
								player.breakBlock(now.getBlock());
								player.playSound(now, Sound.BLOCK_NETHERITE_BLOCK_BREAK, 1f, 1f);
		...
		if (upgrade_left4 && list.size() > 0) {
			long cooldown = Skill.Mining.active4.getRemainUseTime(player);
			if (cooldown > 0) {
				cooldown -= (long) (Skill.Mining.active4.getCooldown(player) * list.size()
						* Skill.Mining.upgrade_left4.getEffect());
				if (cooldown > 0) {
					Skill.Mining.active4.setLastUseTime(player, cooldown / 1000d);
					HashMapStore.setCooldown(player, Skill.Mining.active4, cooldown);
					player.sendMessage(Skill.Mining.active4.getDisplayName() + ChatColor.WHITE + "의 재사용대기시간이 "
							+ list.size() + "% 감소합니다." + ChatColor.GRAY + "(" + String.format("%.1f", cooldown / 1000d)
							+ "s)");
				} else {
		...
		if (main != null && Skill.Mining.upgrade_left2_2.confirmChance(player))
			player.getInventory().setItemInMainHand(main);
	}
